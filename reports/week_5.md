...
The robot remains stationary for 100 samples to collect GNSS data and pre-calculate the trajectory. At sample 101, _init_kalman_filter_ determines the initial mean pose and measurement covariance matrix from the history. Following initialization, the EKF begins continuous pose estimation and trajectory following. In *student_workpsace.m* redundant components, such as the particle filter, path planing were removed for clarity.
